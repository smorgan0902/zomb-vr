...
Shader "AbandonedGarden/PBR_Vegetation" {
 
Properties {
    _MainTex ("Base (RGB) Trans (A)", 2D) = "white" {}
    _Normal ("Normal (RGB)", 2D) = "white" {}
    _E ("Emissive", Range (0, 1.0)) = 0.02
    _M ("Metallic", Range (0, 1.0)) = 0.0
    _S ("Smoothness", Range (0, 1.0)) = 0.3
...
void vert (inout appdata_full v) {
    ...
    const float4 _waveYSize = float4 (0.012, .04, 0.04, 0.1);
    const float4 waveSpeed = float4 (1.2, 2, 1.6, 4.8);
 
    float4 _waveXmove = float4(0.024, 0.04, -0.12, 0.096);
    float4 _waveZmove = float4 (0.006, .02, -0.02, 0.1);
    float4 _waveYmove = float4 (0.003, .01, -0.01, 0.05);
    ...
    float waveAmount = v.texcoord.y * (1 + _ShakeBending);
    s *= waveAmount;
    ...
    v.vertex.xzy += mul ((float3x3)_World2Object, waveMove).xzy * _ShakeBending;
